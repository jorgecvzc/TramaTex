...
### Extensión: Configuraciones de Servicio por Cliente
...
Se propone la creación de una nueva entidad, `PartyServiceConfiguration`, que actúe como un puente entre el módulo `party` y el módulo `product`. Esta entidad se ubicará en el módulo `party`.

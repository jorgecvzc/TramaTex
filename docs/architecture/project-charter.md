...
Las siguientes funcionalidades se documentan para referencia futura y **no se implementarán** en este proyecto (ver [**ADR-004**](../2_architecture/adr/ADR-004-ciclo-vida-desarrollo-mvp.md)):
...
El desarrollo sigue un orden estricto definido en [**ADR-007**](../2_architecture/adr/ADR-007-orden-implementacion-modulos.md) para garantizar la coherencia del dominio.
...
- **Criterio de Aceptación:** Se puede calcular un precio real con datos de clientes y productos. **Cobertura de tests ≥80%** en el dominio de tarificación.
...
- **Criterio de Aceptación:** Se puede crear un pedido estándar de principio a fin. **Cobertura de tests ≥70%** en casos de uso de pedidos.
...
| RNF3 | Mantenibilidad base | TDD + **cobertura ≥75%** (ver [**ADR-011**](../2_architecture/adr/ADR-011-estrategia-testing-coverage.md) para detalles). |
...
- Se alcanza una **cobertura de tests global ≥75%**, con **≥80% en el dominio de tarificación**.

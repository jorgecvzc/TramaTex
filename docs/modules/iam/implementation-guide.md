...
2.  **Definir Interfaces de Repositorio:** Crear `user_repository.go`, `role_repository.go` y `permission_repository.go` en el dominio.
3.  **Implementar Casos de Uso:** Desarrollar la lógica en la capa de aplicación (e.g., `register_user.go`).
4.  **Implementar Repositorios:** Crear las implementaciones de GORM en la capa de infraestructura (roles fijos en MVP).

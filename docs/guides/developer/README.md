# 🛠️ Guías para Desarrolladores

Documentación técnica profunda sobre el framework, patrones de diseño y flujos de trabajo internos.

## 🤖 Sistema de Agentes

- **[Guía de Desarrollo de Agentes](./agent-development-guide.md):** Cómo crear y mantener agentes de IA.

## 📏 Estándares y Convenciones

- **[Convenciones de Nombres de Ramas](./branch-naming-conventions.md):** Estándar para Git.
- **[Convenciones de Nomenclatura](./naming-conventions.md):** Reglas para variables, archivos y funciones.

## ⚙️ Infraestructura y DevOps

- **[CI/CD](./ci-cd.md):** Pipeline de integración y despliegue continuo.
- **[Gestión de Configuración](./configuration-management.md):** Manejo de variables de entorno y secretos.
- **[Comandos de Makefile](./makefile-commands.md):** Automatización de tareas comunes.
- **[Guía de Despliegue](./deployment-guide.md):** Proceso de despliegue del sistema.

## 🧪 Calidad y Seguridad

- **[Guías de Testing](./testing-guidelines.md):** Estrategia y ejecución de pruebas.
- **[Credenciales de Test](./test-credentials.md):** Usuarios y datos para pruebas en desarrollo.
- **[Implementación de Seguridad](./security-implementation-guide.md):** Detalles sobre la defensa en profundidad.
- **[Checklist de Seguridad](./security-compliance-checklist.md):** Lista de verificación para cumplimiento.

## 🏛️ Gobernanza y Deuda

- **[Gobernanza](./governance.md):** Procesos de toma de decisiones técnicas.
- **[Deuda Técnica](./technical-debt.md):** Registro y priorización de mejoras pendientes.

## 📂 Guías Especializadas

- **[Guía de Contribución](./CONTRIBUTING.md):** Cómo contribuir al proyecto.
- **[Control de Versiones](./version-control-guide.md):** Flujo de trabajo Git y convenciones.
- **[Bootstrapping del Proyecto](./project-bootstrapping-guide.md):** Puesta en marcha del proyecto desde cero.
- **[Estructura del Proyecto](./project-structure-details.md):** Detalles de la estructura de archivos y carpetas.
- **[Backend Developer Guide](./backend/README.md):** Guía específica para el desarrollo del backend.
- **[Estrategia de Búsqueda Global](./global-search-strategy.md):** Endpoint, alcance funcional y criterios UX del buscador unificado.

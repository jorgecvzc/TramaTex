...
-   **Tareas de Desarrollo (Sprints y Tareas):** Te dirigirá al agente `init-session.yaml`, que te ayudará a retomar un trabajo pendiente o a empezar una nueva tarea de sprint siguiendo el `Standard Module Development Workflow (SMDW)`.
...
-   **`init-session.yaml`**: Gestiona las sesiones de trabajo. Te permite continuar tareas pendientes de `docs/log/session-log.md` o iniciar nuevas tareas de desarrollo (`_task-template.md`).
...
-   [init-session.yaml](../../../agents/init-session.yaml) (Agente de Gestión de Sesiones)

    ...
    <div class="dashboard-content">
      <header class="page-header">
        <div>
          ...
          <p class="subtitle">Modifica los datos del tipo de trabajo.</p>
        </div>
        <RouterLink to="/mes/work-types" class="btn btn-secondary">Volver</RouterLink>
      </header>

      <section class="card form-card">
        <div v-if="isLoading" class="empty-state">Cargando tipo de trabajo...</div>
        <template v-else>
...
async function submit() {
  ...
  isSaving.value = true
  try {
    await mesApi.updateWorkType(route.params.id as string, {
      name: form.name.trim(),
    ...
    error.value = err.message || 'No se pudo actualizar el tipo de trabajo'
  } finally {
...
onMounted(async () => {
  try {
    const [group, activeTasks] = await Promise.all([
      mesApi.getWorkType(route.params.id as string),
      mesApi.listTasks({ is_active: true }),
    ])

    tasks.value = activeTasks
    form.name = group.name
    form.reference = group.reference || ''
    form.description = group.description || ''
    form.is_active = group.is_active
    form.task_assignments = (group.tasks || []).map((t) => ({
      task_id: t.task_id,
      sequence: t.sequence,
    }))
  } catch (err: any) {
    error.value = err.message || 'No se pudo cargar el tipo de trabajo'
  } finally {

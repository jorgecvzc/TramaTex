<template>
  ...
  <BaseDashboardPage :is-loading="isLoading" class="no-print">
    ...
    <template #header>
      <PageHeader 
        title="Monitoreo de Producción (MES)" 
        :breadcrumbs="[{ label: 'MES', to: '/mes/dashboard' }, { label: 'Monitor' }]"
      >
        <template #icon><span class="material-symbols-outlined">precision_manufacturing</span></template>
        <template #actions>
          <button @click="loadDashboard" class="btn btn-outline" :disabled="isLoading">
            <span class="material-symbols-outlined" :class="{ 'spin': isLoading }">refresh</span>
            Actualizar
          </button>
          <button class="btn btn-primary ml-2" @click="router.push('/mes/terminal')">
      ...
      <div class="mes-sidebar-content">
        ...
        <section class="sidebar-section mb-8">
          ...
          <div class="actions-grid-mini">
            ...
            <RouterLink to="/mes/tasks" class="action-card-sm">
  ...
  <BaseDialog :show="showSetupDialog" title="Configurar Orden de Trabajo" icon="settings" confirm-text="Guardar Configuración" :is-confirming="isSaving" @close="showSetupDialog = false" @confirm="saveSetup">
...
async function loadDashboard() {
  ...
  } catch (err: any) { error.value = 'No se ha podido cargar el dashboard de producción.'; console.error(err) }
...
async function saveSetup() {
  ...
  isSaving.value = true
  try {
    await mesApi.updateWorkOrder(selectedOrder.value.id, { work_setup_id: setupFormData.workSetupId, description: setupFormData.notes })
    showSetupDialog.value = false
    await loadDashboard()
  } catch (err: any) { alert('Error al guardar: ' + err.message) }
  finally { isSaving.value = false }
}
async function startWorkOrder(wo: WorkOrder) { try { await mesApi.updateWorkOrder(wo.id, { status: 'IN_PROGRESS' }); await loadDashboard() } catch (err: any) {} }
async function suspendWorkOrder(wo: WorkOrder) { try { await mesApi.updateWorkOrder(wo.id, { status: 'SUSPENDED' }); await loadDashboard() } catch (err: any) {} }
...
.notice-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem; color: var(--color-text-secondary); }
